...
namespace Voilier_IA_V2
{
    class NodeVoilier : GenericNode
    {
        ...
        {
            int tailleCarre = (int) (9 / MainWindow.mainWindow.tailleCase);
            List<GenericNode> voisins = new List<GenericNode>();
            for (int i = -tailleCarre; i <= tailleCarre; i++)
            {
                for(int j = -tailleCarre; j <= tailleCarre; j++)
                {
                    if ( getDistance(x, y, x + i, y + j) < 9
                        && estCoordonneCorrecte(x + i, y + j) && (i != j || (i==j && i != 0)))
